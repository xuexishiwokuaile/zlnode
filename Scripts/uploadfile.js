...
function eraseFileInServer(paras,formfield,callback){
    if(arguments.length==0){
        alert("invalid number of arguments in eraseFileInServer()");
        return;
    }
    var obj = new Array();
...
function oneUploadAction(server_order,taskId,paras){
    var started=false;
    var valid=true;
    var urlstr = get_appropriate_ws_url(server_order) + "uploadprogress?taskId=" +taskId;
    alert(urlstr);
    var socket_up;
    ...
    socket_up.onopen = function(evt) {
        ...
        uploadToServer(paras.file,paras.callback,paras.additional_args,paras.target_fileid,server_order);
    };
...
// 在所有这些参数中，target_fileid非常特殊，当它为-1时，表示把文件上传到服务器的硬盘中，否则上传到数据库中
// 当target_fileid等于0时，表示建立新的文件，如果不等于0则表示替换在数据库中已经存在的文件
function uploadToServer(file,callback,additional_args,target_fileid,server_id){
    ...
    else if(target_fileid==-1)
        url_str= "uploadDisk.spe";
    else if(target_fileid==0)
        url_str = "uploadFile.spe";
    else
        url_str = "replaceFile.spe?fileid="+target_fileid;
    if(additional_args){
        if(url_str.substr(url_str.length-4,4)==".spe")
